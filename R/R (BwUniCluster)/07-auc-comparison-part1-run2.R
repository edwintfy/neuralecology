...
dir.create(here::here('out_gru_2', 'q_dfs'), showWarnings = FALSE, recursive = TRUE)

load_final_fit <- function(sp) {
  routes <- read_csv('data/cleaned/clean_routes.csv')
  
  nnet_fit <- here::here('out_gru_2', paste0(sp, '_nnet.csv')) %>%
    read_csv %>%
    mutate(method = 'nn') %>%
  ...
  out_name <- here::here('out_gru_2', 'q_dfs', paste0(quant_df$sp.bbs[1], '.csv'))
...
dir.create(here::here('out_gru_2', 'q_dfs'), showWarnings = FALSE)
...
save(ll_dfs, file="ll_dfs_gru_2.RData")




# lstm
species <- list.files('out_lstm_2', '_nnet.csv', full.names = TRUE) %>%
  gsub('out_lstm_2/', '', .) %>%
  gsub('\\_nnet.csv', '', .)

dir.create(here::here('out_lstm_2', 'q_dfs'), showWarnings = FALSE, recursive = TRUE)

load_final_fit <- function(sp) {
  routes <- read_csv('data/cleaned/clean_routes.csv')
  
  nnet_fit <- here::here('out_lstm_2', paste0(sp, '_nnet.csv')) %>%
    read_csv %>%
    mutate(method = 'nn') %>%
    left_join(select(routes, route_id, group))
  ...
  out_name <- here::here('out_lstm_2', 'q_dfs', paste0(quant_df$sp.bbs[1], '.csv'))
  write_csv(quant_df, out_name)
...
# write out q_dfs for each species
dir.create(here::here('out_lstm_2', 'q_dfs'), showWarnings = FALSE)
pboptions(use_lb = TRUE)
cl <- makeCluster(parallel::detectCores())
clusterEvalQ(cl, library(tidyverse))
ll_dfs <- pblapply(species, load_final_fit, cl = cl)
...
save(ll_dfs, file="ll_dfs_lstm_2.RData")




# rnn
species <- list.files('out_rnn_2', '_nnet.csv', full.names = TRUE) %>%
  gsub('out_rnn_2/', '', .) %>%
  gsub('\\_nnet.csv', '', .)

dir.create(here::here('out_rnn_2', 'q_dfs'), showWarnings = FALSE, recursive = TRUE)

load_final_fit <- function(sp) {
  routes <- read_csv('data/cleaned/clean_routes.csv')
  
  nnet_fit <- here::here('out_rnn_2', paste0(sp, '_nnet.csv')) %>%
    read_csv %>%
    mutate(method = 'nn') %>%
    left_join(select(routes, route_id, group))
  ...
  out_name <- here::here('out_rnn_2', 'q_dfs', paste0(quant_df$sp.bbs[1], '.csv'))
  write_csv(quant_df, out_name)
...
# write out q_dfs for each species
dir.create(here::here('out_rnn_2', 'q_dfs'), showWarnings = FALSE)
pboptions(use_lb = TRUE)
cl <- makeCluster(parallel::detectCores())
clusterEvalQ(cl, library(tidyverse))
ll_dfs <- pblapply(species, load_final_fit, cl = cl)
...
save(ll_dfs, file="ll_dfs_rnn_2.RData")
